...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check rate limits and decide which API to use
    if (rateLimiter.canUseGemini()) {
      // Try Gemini first
      try {
    ...
    if (!explanation) {
      if (!rateLimiter.canUseGemini() && !rateLimiter.canUseGroq()) {
        return NextResponse.json({
          error: 'Your daily rate limit of 30 requests has been reached for both AI services. Please come back tomorrow to continue using the app.'
        }, { status: 429 });
      } else {
